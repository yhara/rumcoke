...
var expandQq = function (a) {
    return !_.isArray(a) ? a : cons(Sym('append'), a.map(function (x) {
        return x[0] === Sym('quasiquote') ? raise('Nested quasiquote is not supported yet') : x[0] === Sym('unquote') ? append([Sym('array')], [x[1]]) : x[0] === Sym('unquote-splicing') ? x[1] : isSymbol(x) ? append([Sym('array')], [append([Sym('quote')], [x])]) : _.isArray(x) ? append([Sym('array')], [expandQq(x)]) : x;
    }));
};
...
var macros = {
        ...
        'case': function (v) {
            return [
                Sym('raw-js-ast'),
                ast('SwitchStatement', {
                    'discriminant': translateExpr(v[1]),
