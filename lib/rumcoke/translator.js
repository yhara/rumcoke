// Generated by RumCoke 0.2.1.dev
...
var isStatement = function (ast) {
    return ast.type.match(/ForStatement|ThrowStatement|WhileStatement/) || ast.type === 'IfStatement' && _.isNull(ast.alternate);
};
var stmtsWithReturn = function (bodyExprs) {
    return bodyExprs.map(function (bodyItem, idx) {
        return idx === bodyExprs.length - 1 ? function () {
            var novalAst = convertNode(bodyItem, false);
            return isStatement(novalAst) ? novalAst : ast('ReturnStatement', {'argument': convertValue(bodyItem)});
        }.call(this) : convertStmt(bodyItem);
    });
};
var blockStmtWithReturn = function (bodyExprs) {
    return ast('BlockStatement', {'body': stmtsWithReturn(bodyExprs)});
};
var functionExpression = function (paramsAry, defaults, body) {
...
var functionWithReturn = function (paramsAry, defaults, bodyExprs) {
    return functionExpression(paramsAry, defaults, blockStmtWithReturn(bodyExprs));
};
...
var callWithCurrentThis = function (funcAst) {
    return ast('CallExpression', {
        'arguments': [ast('ThisExpression')],
...
var syntaxes = {
        ...
        'case': function (v, isValueNeeded) {
            var bodyConverter = isValueNeeded ? stmtsWithReturn : convertStmts;
            var switchAst = ast('SwitchStatement', {
                    'discriminant': convertValue(v[1]),
                    'cases': flatMap(v.slice(2), function (clause) {
                        var cases = clause[0];
                        return Sym('else') === cases ? ast('SwitchCase', {
                            'test': null,
                            'consequent': bodyConverter(clause.slice(1))
                        }) : function () {
                            var initCases = _.initial(cases);
                            var lastCase = _.last(cases);
                            var body = clause.slice(1);
                            return append(_.map(initCases, function (test) {
                                return ast('SwitchCase', {
                                    'test': convertValue(test),
                                    'consequent': []
                                });
                            }), [ast('SwitchCase', {
                                    'test': convertValue(lastCase),
                                    'consequent': append(bodyConverter(body), ast('BreakStatement', {'label': null}))
                                })]);
                        }.call(this);
                    })
                });
            return isValueNeeded ? callWithCurrentThis(functionExpression([], [], ast('BlockStatement', {'body': [switchAst]}))) : switchAst;
        },
        'begin': function (v, isValueNeeded) {
            return isValueNeeded ? callWithCurrentThis(functionWithReturn([], [], v.slice(1))) : ast('BlockStatement', {'body': _.map(v.slice(1), convertStmt)});
        },
        'and': function (v) {
        ...
        'inc!': function (v) {
            raiseIf(v.length === 0 || v.length > 3, {
                'msg': 'malformed inc!',
                'v': v
            });
            return _.isUndefined(v[2]) ? ast('UpdateExpression', {
                'operator': '++',
                'argument': convertValue(v[1]),
                'prefix': false
            }) : ast('AssignmentExpression', {
                'operator': '+=',
                'left': ast('Identifier', convertValue(v[1])),
                'right': convertValue(v[2])
            });
        },
        'dec!': function (v) {
        ...
        '-': function (v) {
            return function () {
                ...
                case 1:
                    return raise({
                        'msg': '`-` needs at least one argument',
                        'v': v
                    });
                    break;
                case 2:
...
var convertNode = function (v, isValueNeeded) {
    ...
    }.call(this) : isObjExpr(v) ? ast('ObjectExpression', {'properties': _.map(_.pairs(v.content), function (pair) {
            return ast('Property', {
                'kind': 'init',
...
var macros = {
        ...
        'when': function (v) {
            return append([Sym('if')], [v[1]], [append([Sym('begin')], v.slice(2))]);
        },
        'unless': function (v) {
            return append([Sym('if')], [append([Sym('not')], [v[1]])], [append([Sym('begin')], v.slice(2))]);
        },
...
var expandMacros = function (v, mod) {
    ...
    return _.isArray(v) ? function () {
        ...
        return isSymbol(car) ? syntaxes[car.name] ? _.contains([
            ...
            Sym('<='),
            Sym('>=')
        ], car) ? _.map(v, expandInner) : Sym('..') === car ? append([
            Sym('..'),
            ...
            return macro ? function () {
                ...
                return ret;
            }.call(this) : v.map(expandInner);
        }.call(this) : v.map(expandInner);
    }.call(this) : isObjExpr(v) ? function () {
        var ret = new Object();
